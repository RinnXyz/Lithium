...
import com.rinn.engine.client.LithiumNewProcess
...
class QuickShellViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private var process: LithiumNewProcess? = null
    ...
    fun stop(fromUser: Boolean = true) {
        if (isShellRestrictionEnabled) Lithium.newProcess("kill -TERM -$pid")
        debounceJob?.cancel()
        debounceJob = null
        job?.cancel()
        job = null
    ...
    fun runShell() {
        ...
        launchSafely(cmd) {
            ...
            process = Lithium.newProcess(
                getQuickCmd(
                    cmd = cmd,
                    useBusybox = isCompatModeEnabled,
                    withPid = true
                ),
                Lithium.getEnvironment(), null
            )
