package com.rinn.axmanager.ui.screen
...
import com.rinn.axmanager.BuildConfig
import com.rinn.axmanager.R
import com.rinn.axmanager.ui.component.ExtraLabel
import com.rinn.axmanager.ui.component.ExtraLabelDefaults
import com.rinn.axmanager.ui.component.rememberConfirmDialog
import com.rinn.axmanager.ui.component.rememberLoadingDialog
import com.rinn.axmanager.ui.component.scaleDp
import com.rinn.axmanager.ui.util.checkNewVersion
import com.rinn.axmanager.ui.util.module.LatestVersionInfo
import com.rinn.axmanager.ui.viewmodel.AdbViewModel
import com.rinn.axmanager.ui.viewmodel.AppsViewModel
import com.rinn.axmanager.ui.viewmodel.PluginViewModel
import com.rinn.axmanager.ui.viewmodel.QuickShellViewModel
import com.rinn.axmanager.ui.viewmodel.ViewModelGlobal
import com.rinn.engine.client.Lithium
import com.rinn.engine.client.PluginService
import com.rinn.engine.implementation.LithiumService
...
fun HomeScreen(navigator: DestinationsNavigator, viewModelGlobal: ViewModelGlobal) {
    ...
    val lithiumInfo = adbViewModel.lithiumInfo

    LaunchedEffect(lithiumInfo) {
        if (lithiumInfo.isNeedUpdate()) {
            Log.d(
                "AxManager",
                "NeedUpdate ${Lithium.getInfo().versionCode} > ${LithiumService.VERSION_CODE}"
            )
            adbViewModel.isUpdating = true
            Lithium.newProcess(QuickShellViewModel.getQuickCmd(Starter.internalCommand), null, null)
        } else {
        ...
        topBar = {
            TopAppBar(
                ...
                actions = {
                    ...
                    AnimatedVisibility(visible = adbViewModel.lithiumInfo.isRunning()) {
                        Card(
...
fun SupportCard() {
    ...
    val githubRinnXyz = "https://github.com/RinnXyz/LithiumManager"

    ElevatedCard(
        onClick = {
            uriHandler.openUri(githubRinnXyz)
        }
    ) {
        Row(
            modifier = Modifier
...
fun StatusCard(
    ...
    Log.d("LithiumManager", "NeedUpdate: ${lithiumInfo.isNeedUpdate()}")
        ...
        colors = CardDefaults.elevatedCardColors(
            containerColor = run {
                when {
                    ...
                    lithiumInfo.isNeedExtraStep() -> MaterialTheme.colorScheme.errorContainer
                    lithiumInfo.isRunning() -> MaterialTheme.colorScheme.primaryContainer
        ...
        Box(
            modifier = Modifier
                ...
                .clickable {
                    ...
                    if (lithiumInfo.isNeedExtraStep()) {
                        uriHandler.openUri(extraStepUrl)
                        return@clickable
                    }
                    onClick(lithiumInfo.isRunning())
                }
        ) {
            ...
            when {
                adbViewModel.isUpdating -> {
                    ...
                    Column(
                        ...
                        Text(
                            text = "Version: ${lithiumInfo.versionCode} > ${LithiumService.VERSION_CODE}",
                            style = MaterialTheme.typography.bodySmall
                        )
                    }
                }

                lithiumInfo.isNeedExtraStep() -> {
                    Box(
                        modifier = Modifier
                            .fillMaxSize()
                            .offset(42.dp, 60.dp)
                ...
                lithiumInfo.isRunning() -> {
                    Box(
                        ...
                        Icon(
                            ...
                            painter = painterResource(com.rinn.engine.R.drawable.ic_lithium),
                            contentDescription = null
                        )
                    }
                    Box(
                    ...
                    Column(
                        ...
                        Text(
                            text = "Version: ${lithiumInfo.versionCode} | Pid: ${lithiumInfo.pid}",
                            style = MaterialTheme.typography.bodySmall
                        )
...
fun InfoCard(adbViewModel: AdbViewModel) {
    ...
    ElevatedCard(
        ...
        Column(
            ...
            LaunchedEffect(Unit) {
                while (true) {
                    time = SystemClock.elapsedRealtime() - lithiumInfo.starting
                    delay(1000)
                }
            }
            ...
            InfoCardItem(
                ...
                content = lithiumInfo.selinuxContext,
...
fun IssueReportCard() {
    val uriHandler = LocalUriHandler.current
    val githubIssueUrl = "https://github.com/RinnXyz/LithiumManager/issues"
    val telegramUrl = "https://t.me/lithium_manager"
