package com.rinn.lithium.data

import android.content.ContentResolver
import android.provider.Settings
import com.rinn.engine.Environment
import com.rinn.engine.client.Lithium
import com.rinn.engine.client.PluginService
import com.rinn.engine.implementation.LithiumService.Companion.TYPE_NEW_ENV
...
class SettingsRepository(private val contentResolver: ContentResolver) {
    ...
    fun putSetting(type: SettingType, key: String, value: String): Boolean {
        ...
        return try {
            Lithium.newProcess(arrayOf("settings", "put", alias, key, value)).waitFor() == 0
        } catch (e: Exception) {
            e.printStackTrace()
            false
        }
    ...
    fun deleteSetting(type: SettingType, key: String): Boolean {
        ...
        return try {
            Lithium.newProcess(arrayOf("settings", "delete", alias, key)).waitFor() == 0
        } catch (e: Exception) {
            e.printStackTrace()
            false
        }
    ...
    private fun querySettings(type: SettingType): Map<String, String> {
        ...
        try {
            val process = Lithium.newProcess(arrayOf("settings", "list", alias))
    ...
    private fun getAndroidProp(key: String): String? {
        return try {
            val process = Lithium.newProcess(arrayOf(PluginService.RESETPROP, key))
    ...
    private fun setAndroidProp(key: String, value: String): Boolean {
        return try {
            Lithium.newProcess(arrayOf(PluginService.RESETPROP, key, value)).waitFor() == 0
        } catch (e: Exception) {
            e.printStackTrace()
            false
        }
    ...
    private fun getAllAndroidProps(): Map<String, String> {
        val map = mutableMapOf<String, String>()
        try {
            val process = Lithium.newProcess(PluginService.RESETPROP)
            process.inputStream.bufferedReader().useLines { lines ->
                lines.forEach { line ->
    ...
    private fun getLinuxEnv(key: String): String? = Lithium.getEnvironment().envMap[key]


    private val envMap: Map<String, String>
        get() = Lithium.getEnvironment(TYPE_NEW_ENV).envMap

    private fun setLinuxEnv(key: String, value: String): Boolean {
        val newEnvMap = HashMap(envMap) // mutable
        newEnvMap[key] = value
        return try {
            Lithium.setNewEnvironment(Environment(newEnvMap, true))
            true
        } catch (e: Exception) {
            e.printStackTrace()
            false
    ...
    private fun getAllLinuxEnv(): Map<String, String> {
        return Lithium.getEnvironment().envMap
    }
}
